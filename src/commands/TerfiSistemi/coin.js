...
module.exports = {
  ...
  return message.channel.send(`Bu komutu kullanabilmek için ayarlanan sahip rolüne sahip olmalısınız!`).then(x => x.delete({timeout: 5000}));
    ...
    if (args[0] === "ekle" || args[0] === "add") {
      ...
      if (coinData && client.ranks.some(x => coinData.coin >= x.coin && !member.roles.cache.has(x.role))) {
        ...
        message.guild.channels.cache.get(ayar.rankLog).send(embed.setDescription(`${member.toString()} üyesine ${message.member.toString()} tarafından **${count}** adet coin eklendi ve kişiye ${roles.filter(x => roles.indexOf(x) === roles.length-1).map(x => `<@&${x.role}>`).join("\n")} rolleri verildi!`));
      }
